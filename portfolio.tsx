...
const Portfolio = () => {
  ...
  const totalPosts = facebookPostUrls.length;
  const fbLoading = totalPosts > 0 && loadedPosts < totalPosts;
          ...
          <section className="mb-16">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto justify-items-center">
              {facebookPostUrls.map((url, index) => (
                <FacebookPost key={index} url={url} width={450} onLoad={() => setLoadedPosts(prev => prev + 1)} />
              ))}
            </div>
          </section>
        )}
...
const Loader = () => {
  return (
    <div className="flex justify-center mb-6">
      <InfinitySpin width="200" color="#9CA3AF" />
    </div>
  );
};
